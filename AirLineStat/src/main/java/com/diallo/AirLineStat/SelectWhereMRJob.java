...
public class SelectWhereMRJob extends Configured implements Tool{
	...
	public static class SelectMapper extends Mapper<LongWritable, Text, NullWritable, Text>{
		...
		protected void setup(
			...
			this.minimumDelay = context.getConfiguration().getInt("map.where.delay", 0);
		}
		...
		protected void map(LongWritable key, Text value,
			...
			if (!AirlineDataUtil.isHeader(value)){
				String[] tousLesChamps = value.toString().split(",");
				if (AirlineDataUtil.parseMinutes(AirlineDataUtil.getArrivalDelay(tousLesChamps), 0) > this.minimumDelay){
